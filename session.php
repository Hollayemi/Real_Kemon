...
if(isset($_SESSION['user_info_id']) || isset($_SESSION['user_info_id2'])){
    ...
    if(isset($_SESSION['user_info_id2'])){
        ...
        $sql="SELECT * FROM agent WHERE withdrawableCashFlow >0";
        $run=mysqli_query($mysqli,$sql);
        $queryRun = mysqli_num_rows($run);
        $_SESSION['counting5'] = $queryRun;

        $sql="SELECT * FROM agent WHERE Total_reg>10";
        $run=mysqli_query($mysqli,$sql);
        $queryRun = mysqli_num_rows($run);
        $_SESSION['bestReferrals'] = $queryRun;

        $sql="SELECT type_of_sub FROM subscribers WHERE type_of_sub='12'";
        $run=mysqli_query($mysqli,$sql);
        $queryRun = mysqli_num_rows($run);
        $_SESSION['type_of_sub_for_year'] = $queryRun;

        $sql="SELECT type_of_sub FROM subscribers WHERE type_of_sub='3'";
        $run=mysqli_query($mysqli,$sql);
        $queryRun = mysqli_num_rows($run);
        $_SESSION['type_of_sub_for_3months'] = $queryRun;

        $sql="SELECT type_of_sub FROM subscribers WHERE type_of_sub='6'";
        $run=mysqli_query($mysqli,$sql);
        $queryRun = mysqli_num_rows($run);
...
//        echo $_SESSION['rems'];
    ...
    //         $newExpiry = abs(floor($dif/(60 * 60 * 24))); 
    //         echo "__".$newExpiry."___";
    //         echo $DL."==";
    //         $real_exp_date = $DL + $newExpiry;

    //         $CC = $real_exp_date - $newExpiry;

    //         echo $real_exp_date."real expire date". "<br>";
    //         $nn = 1;
    //         $sub_sql = "UPDATE users SET Subscribed = '$nn' WHERE  id = '$user_id'";
    //         mysqli_query($mysqli,$sub_sql);
            
    //         $pop = $rem_row['Date_expired'];
    //         $sub_sql = "UPDATE subscribers SET Date_expired = '$pop', Days_left = '$real_exp_date'  WHERE  id = '$user_id'";
    //         mysqli_query($mysqli,$sub_sql);
            

            
    //         $ss = 2;
    //         $sub_sql = "UPDATE users SET Subscribed = '$ss' WHERE  id = '$user_id'";
    //         mysqli_query($mysqli,$sub_sql);
    //         $nn = 3;
    //         $del_sub = "DELETE FROM subscribers WHERE Days_left = '$nn'";
    //         mysqli_query($mysqli,$del_sub);
        
    //         $rem_sql2 = "UPDATE subscribers SET Days_left = '$real_exp_date' WHERE id = '$user_id'";
    //         mysqli_query($mysqli,$rem_sql2);
    //     }
